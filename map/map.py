...
try:
    ...
    print(
        "Run `pip install mapbox-vector-tile protobuf requests` to fix your dependencies."
    )
    print(
        "See https://github.com/Toblerity/Shapely#installing-shapely-16b2 for Shapely install."
    )
    sys.exit(0)
...
class EnterLocation(Frame):
    ...
    def __init__(self, screen, longitude, latitude, on_ok):
        super(EnterLocation, self).__init__(
            ...
            7,
            40,
            data={"long": str(longitude), "lat": str(latitude)},
            name="loc",
        ...
        layout.add_widget(
            Text(label="Longitude:", name="long", validator=r"^[-]?\d+?\.\d+?$"), 1
        )
        layout.add_widget(
            Text(label="Latitude:", name="lat", validator=r"^[-]?\d+?\.\d+?$"), 1
        )
        layout.add_widget(Divider(draw_line=False), 1)
...
class Map(Effect):
        ...
        while self._running:
            ...
            for x, y, z in [
                ...
                try:
                    ...
                    if z == 0 and (
                        left > self._screen.width
                        or left + self._size * 2 < 0
                        or top > self._screen.height
                        or top + self._size < 0
                    ):
                        continue
    ...
    def _update(self, frame_no):
        ...
        if self._tiles:
            ...
            bg = (
                253 if self._screen.unicode_aware and self._screen.colours >= 256 else 0
            )
        ...
        if count == 0:
            self._screen.centre(
                " Loading - please wait... ", self._screen.height // 2, 1
            )
